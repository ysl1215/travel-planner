...
import { Globe, Clock, TrendingUp, CheckCircle, Plane, Train, Loader2, ChevronDown, ChevronUp } from "lucide-react";
...
interface DestinationCardProps {
  ...
  /** Home city name (for train estimate lookup) */
  originCity?: string;
  ...
  /** Currency */
  currency?: string;
  /** Pre-fetched price data from parallel "Compare All" fetch */
  prefetchedPrices?: { flights: FlightOffer[]; priceLevel: string; error: string | null };
  /** Travel budget allocation from the slider (used to recalculate budget fit with real prices) */
  travelBudget?: number;
  /** Pre-fetched train estimate from parent-level batch fetch */
  prefetchedTrainEstimate?: any;
  /** Pre-fetched hotel estimate from parent-level batch fetch */
  prefetchedHotelEstimate?: any;
}
...
export default function DestinationCard({
  ...
  prefetchedPrices,
  travelBudget,
  prefetchedTrainEstimate,
  prefetchedHotelEstimate,
}: DestinationCardProps) {
  const [priceData, setPriceData] = useState<{
    flights: FlightOffer[];
    priceLevel: string;
    error: string | null;
  } | null>(prefetchedPrices ?? null);
  const [loadingPrices, setLoadingPrices] = useState(false);
  const [showPrices, setShowPrices] = useState(!!prefetchedPrices);

  // Derive destination airport code — prefer AI-provided code, fall back to static map
  const destAirport = destination.airportCode || cityToAirport(destination.city);

  // Train estimate — use prefetched live data from parent, fall back to static
  const staticTrainEstimate = originCity
    ? getTrainEstimate(originCity, destination.city)
    : null;
  const trainEstimate = prefetchedTrainEstimate ?? staticTrainEstimate;

  // Accommodation estimate — use prefetched live data from parent, fall back to static
  const staticAccomEstimate = getAccomEstimate(destination.city);
  const accomEstimate = prefetchedHotelEstimate ?? staticAccomEstimate;

  // Sync prefetched prices when they arrive from parent
  useEffect(() => {
    if (prefetchedPrices) {
      setPriceData(prefetchedPrices);
      setShowPrices(true);
    }
  }, [prefetchedPrices]);
  ...
  const tripNights = departureDate && returnDate
    ? Math.max(1, Math.round((new Date(returnDate).getTime() - new Date(departureDate).getTime()) / 86_400_000))
    : null;

  // Recalculate budget fit using real flight prices when available
  const cheapestPrice = priceData?.flights?.length
    ? Math.min(...priceData.flights.filter((f) => f.price > 0).map((f) => f.price))
    : null;

  // Total flight cost for all travelers, not just one ticket
  const totalFlightCost = cheapestPrice !== null ? cheapestPrice * (adults ?? 1) : null;

  const realBudgetFit: Destination["estimatedBudgetFit"] | null =
    totalFlightCost !== null && travelBudget && travelBudget > 0
      ? totalFlightCost <= travelBudget * 0.6
        ? "excellent"
        : totalFlightCost <= travelBudget
        ? "good"
        : "stretch"
      : null;

  const activeBudgetFit = realBudgetFit ?? destination.estimatedBudgetFit;
    ...
    >
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full border font-medium ${budgetFitColors[activeBudgetFit]}`}
              >
                {realBudgetFit ? `${budgetFitLabels[activeBudgetFit]} ✓` : budgetFitLabels[activeBudgetFit]}
              </span>
            </div>
      ...
      <div className="p-5">
        ...
        {canFetchPrices ? (
          ...
          <div className="mb-4">
            <p className="text-xs text-gray-400 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 flex items-center gap-1.5">
              <Plane className="w-3.5 h-3.5" />
              {!originAirport
                ? "Price check unavailable — home city airport not recognized"
                : !destAirport
                ? `Price check unavailable — no airport code for ${destination.city}`
                : "Price check unavailable — departure date required"}
            </p>
          </div>
        )}
        ...
        {trainEstimate && (
          <div className="mb-4 text-xs bg-violet-50 border border-violet-200 rounded-xl px-3 py-2">
            ...
            <div className="text-violet-600">
              ...
              {"duration" in trainEstimate && trainEstimate.duration && (
                <span className="text-violet-500 ml-1">· {trainEstimate.duration}</span>
              )}
              {"note" in trainEstimate && (
                <span className="text-violet-400 ml-1">· {trainEstimate.note}</span>
              )}
            </div>
          </div>
        )}
        ...
        {accomEstimate && (
          <div className="mb-4 text-xs bg-teal-50 border border-teal-200 rounded-xl px-3 py-2">
            ...
            <div className="flex gap-3 text-teal-600">
              ...
              <span>🚪 Budget ~{accomEstimate.budget}</span>
              <span>🏩 Mid-range ~{accomEstimate.midrange}</span>
            </div>
            {tripNights && (
              <div className="text-teal-500 mt-1">
                {tripNights} nights · budget total ~{accomEstimate.budget * tripNights * (adults ?? 1)}
                <span className="opacity-60 ml-1">(est.)</span>
              </div>
            )}
          </div>
        )}
