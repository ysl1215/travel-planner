  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === "itinerary" && (
        <div className="space-y-4">
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
            💡 Costs shown are AI estimates — verify locally before budgeting.
          </p>

          {/* Clusters — shown when multi-attraction areas exist */}
          {itinerary.clusters && itinerary.clusters.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                🗺️ Plan your time in key areas
              </h3>
              {itinerary.clusters.map((c, i) => (
                <ClusterCard key={i} cluster={c} />
              ))}
            </div>
          )}
          {itinerary.days.slice(0, visibleDays).map((day) => (
            <DayCard key={day.day} day={day} />
          ))}

          {visibleDays < itinerary.days.length && (
            <button
              onClick={() => setVisibleDays(itinerary.days.length)}
              className="w-full py-3 rounded-2xl border border-dashed border-gray-300 text-sm text-gray-500 hover:text-gray-700 hover:border-gray-400 transition-colors"
            >
      ...
      {activeTab === "attractions" && (
        <div className="space-y-4">
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
            💡 Costs and wait times are AI estimates — verify before visiting.
          </p>
      ...
      {activeTab === "food" && (
        <div className="space-y-4">
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
            💡 Price ranges are AI estimates — actual prices may vary.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
