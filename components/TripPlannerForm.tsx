...
export default function TripPlannerForm({ onSubmit, isLoading }: TripPlannerFormProps) {
  ...
  // Persist travel profile to localStorage
  useEffect(() => {
    try {
      const saved = localStorage.getItem("travel_profile");
      if (saved) {
        const { travelPriorities, pastTrips } = JSON.parse(saved);
        setForm((f) => ({ ...f, travelPriorities: travelPriorities || "", pastTrips: pastTrips || "" }));
      }
    } catch {}
  }, []);

  useEffect(() => {
    if (form.travelPriorities || form.pastTrips) {
      localStorage.setItem("travel_profile", JSON.stringify({
        travelPriorities: form.travelPriorities,
        pastTrips: form.pastTrips,
      }));
    }
  }, [form.travelPriorities, form.pastTrips]);
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm text-gray-600 mb-1">
              What matters most to you on this trip? <span className="text-red-500">*</span>
            </label>
            <textarea
              placeholder="e.g. I want remote mountain trails with no crowds, eat at places only locals know, spend a full day on one incredible hike rather than rush through 5 viewpoints..."
              value={form.travelPriorities}
              onChange={(e) => setForm({ ...form, travelPriorities: e.target.value })}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-gray-900 focus:outline-none focus:ring-2 focus:ring-rose-400 min-h-[80px]"
              required
              rows={3}
            />
            <p className="text-xs text-gray-400 mt-1">This is the #1 signal for personalisation — be specific!</p>
          </div>
          <div>
            <label className="block text-sm text-gray-600 mb-1">
              Past trips you loved or hated (and why)
            </label>
            <textarea
              placeholder="e.g. Loved the Faroe Islands (remote, dramatic, zero crowds). Hated Santorini (overpriced, packed, Instagram-optimised)."
              value={form.pastTrips}
              onChange={(e) => setForm({ ...form, pastTrips: e.target.value })}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-gray-900 focus:outline-none focus:ring-2 focus:ring-rose-400 min-h-[60px]"
              rows={2}
            />
            <p className="text-xs text-gray-400 mt-1">Helps calibrate — match the &quot;loved&quot; vibe, avoid the &quot;hated&quot; vibe</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            <button
              type="button"
              onClick={() => setForm({ ...form, preferHiddenGems: !form.preferHiddenGems })}
              className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl border-2 transition-all text-left ${
                form.preferHiddenGems
                  ? "border-amber-400 bg-amber-50 text-amber-800"
                  : "border-gray-200 bg-gray-50 text-gray-600 hover:border-amber-300"
              }`}
            >
              <span className="text-xl">{form.preferHiddenGems ? "🗺️" : "🏛️"}</span>
              <div>
                <div className="text-sm font-medium">
                  {form.preferHiddenGems ? "Off-the-beaten-path mode ON" : "Prefer off-the-beaten-path destinations"}
                </div>
                <div className="text-xs opacity-70">
                  {form.preferHiddenGems
                    ? "Prioritising hidden gems, local spots, and lesser-known destinations"
                    : "Toggle to favour hidden gems over tourist hotspots"}
                </div>
              </div>
            </button>
          </div>
